...
from model import NexusDecoder
...
class NexusSegmentationModel(nn.Module):
...
def main():
    ...
    decoder = NexusDecoder(
        in_channels=in_channels, num_classes=n_classes, TH=TH, TW=TW
    )
    pipeline = NexusSegmentationModel(backbone, decoder, H, W, layer_indices).to(device)
    ...
    if args.resume_phase2:
        ...
        save_path = os.path.join(output_dir, "nexus_model.pth")
    ...
    for epoch in range(start_epoch, n_epochs):
        if epoch == phase1_epochs:
            ...
            lora_params = inject_lora(pipe.backbone, rank=16, target_blocks=6)
